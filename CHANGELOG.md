...
* Bug fix: Notification emitter now checks whether a moved node is part of partition of not. If moved node is a floating (not part of a partition) node and replaces another node, then ChildReplacedNotification is emitter. If there is no replacement, then ChildAddedNotification is emitted.
### Removed
### Deprecated
### Security
