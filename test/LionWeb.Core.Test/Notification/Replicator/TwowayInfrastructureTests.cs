...
public class TwowayInfrastructureTests : TwowayReplicatorTestsBase
{
    ...
    {
        var fieldInfoFilter =
            typeof(MultipartNotificationHandler).GetRuntimeFields().First(f => f.Name == "_lastHandler");
        var filter = (IdFilteringNotificationFilter)fieldInfoFilter.GetValue(replicator);

        var fieldInfoNotificationIds = typeof(IdFilteringNotificationFilter).GetRuntimeFields()
            .First(f => f.Name == "_notificationIds");
