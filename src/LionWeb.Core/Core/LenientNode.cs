...
public class LenientNode : NodeBase, INode
{
    ...
    private sealed class LenientNodeEqualityComparer : IEqualityComparer<LenientNode>
    {
        public bool Equals(LenientNode? x, LenientNode? y)
        {
            ...
            if (x._classifier != null && y._classifier != null && !x._classifier.EqualsIdentity(y._classifier))
                return false;
            if (x._classifier == null || y._classifier == null)
                return false;

            if (!x._annotations.Cast<LenientNode>().SequenceEqual(y._annotations.Cast<LenientNode>(), this))
                return false;

            var xFeatureKeys = x.FeatureKeys.OrderBy(f => f.Key).ToList();
            if (!xFeatureKeys.SequenceEqual(y.FeatureKeys.OrderBy(f => f.Key), new FeatureIdentityComparer()))
                return false;

            return xFeatureKeys.Select(f =>
                {
                    if (x.TryGet(f, out var xVal) && y.TryGet(f, out var yVal))
                        return (f, xVal, yVal);

                    return (null, null, null);
                })
                .Where(p => p.xVal != null || p.yVal != null)
                .All(p => p switch
                {
                    var (_, a, b) when a == b => true,
                    (_, null, not null) => false,
                    (_, not null, null) => false,
                    (Containment, LenientNode a, LenientNode b) => Equals(a, b),
                    (Containment, IReadableNode a, IReadableNode b) => a.Equals(b),
                    (Containment, IEnumerable<LenientNode> a, IEnumerable<LenientNode> b) => a.SequenceEqual(b, this),
                    (_, IEnumerable a, IEnumerable b) => a.Cast<object?>().SequenceEqual(b.Cast<object?>()),
                    (_, { } a, { } b) => a.Equals(b),
                    _ => false
                });
        }

        public int GetHashCode(LenientNode obj)
        {
            var hashCode = new HashCode();

            hashCode.Add(obj._classifier?.GetHashCodeIdentity());

            foreach (var ann in obj._annotations.Cast<LenientNode>())
            {
                hashCode.Add(ann, this);
            }

            foreach ((Feature feature, object? value) in obj._featureValues)
            {
                hashCode.Add(feature.GetHashCodeIdentity());
                switch (value)
                {
                    case null:
                        hashCode.Add(0);
                        break;
                    case LenientNode l when feature is Containment:
                        hashCode.Add(GetHashCode(l));
                        break;
                    case IEnumerable<LenientNode> e when feature is Containment:
                        foreach (var child in e)
                        {
                            hashCode.Add(GetHashCode(child));
                        }
    ...
    public static IEqualityComparer<LenientNode> LenientNodeComparer { get; } = new LenientNodeEqualityComparer();
}